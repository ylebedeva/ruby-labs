...
class Assignment
  ...
  def create_todolist(params)
      ...
      # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
      if (!params.nil?)
        params[:list_name] = params[:name]
        params[:list_due_date] = params[:due_date]
        params.slice!(:list_name, :list_due_date)
      end
      TodoList.new(params).save()
  end
  ...
  def find_allusers(offset, limit)
      ...
      User.order(:updated_at => :asc).offset(offset).limit(limit).all.to_a
  end
  ...
  def find_alllists(offset, limit)
      ...
      TodoList.order(:list_due_date => :desc).offset(offset).limit(limit).all.to_a
  end
  ...
  def find_user_byname(username)
      ...
      User.where(:username => username).all.to_a
  end 
  ...
  def find_todolist_byname(name)
      ...
      TodoList.where(:list_name => name).all.to_a
  end     
